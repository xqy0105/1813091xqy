$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'doc_manage/docmodifyapply/list',
        datatype: "json",
        colModel: [
            {label: '', name: '', index: 'action', width: 120, sortable:false,formatter: cmgStateFormat},
			//{ label: 'docModifyApplyId', name: 'docModifyApplyId', index: 'doc_modify_apply_id', width: 50, key: true },
			{ label: '档案ID', name: 'docId', index: 'doc_id', width: 80 },
			{label: '档案名', name: 'docName', index: 'doc_name', width: 80},
			//{ label: '审批人id', name: 'approverId', index: 'approver_id', width: 80 },
            { label: '审批人', name: 'approverName', index: 'approver_name', width: 80 },
            { label: '修改申请人', name: 'ownUserName', index: 'ownUser_name', width: 80 },
			//{ label: '档案管理员ID', name: 'ownUserId', index: 'own_user_id', width: 80 },
...
function cmgStateFormat(cellvalue, options, rowObject) {
    ...
    if(rowObject.approveStatus === 1){
        ...
        "<button type=\"button\" class='btn btn-warning' style='line-height: 0.6' onclick=\"RejectSingle(" + rowObject.docModifyApplyId + ")\">驳回</button>" +
            "<button type=\"button\" class='btn btn-primary' style='line-height: 0.6' onclick=\"ApproveSingle(" + rowObject.docModifyApplyId + ")\">确认申请</button>"+
...
var vm = new Vue({
	...
	methods: {
		...
		update: function (event) {
			var docModifyApplyId = getSelectedRow();
			if(docModifyApplyId == null){
				return ;
			}
			vm.showList = false;
            vm.title = "修改";

            vm.getInfo(docModifyApplyId)
		},
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.docModifyApply.docModifyApplyId == null ? "doc_manage/docmodifyapply/save" : "doc_manage/docmodifyapply/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    data: JSON.stringify(vm.docModifyApply),
                    success: function(r){
                        if(r.code === 0){
		...
		del: function (event) {
			var docModifyApplyIds = getSelectedRows();
			if(docModifyApplyIds == null){
				return ;
			}
			var lock = false;
            layer.Modify('确定要删除选中的记录？', {
                btn: ['确定','取消'] //按钮
            }, function(){
               if(!lock) {
                    lock = true;
		            $.ajax({
                        type: "POST",
                        url: baseURL + "doc_manage/docmodifyapply/delete",
                        contentType: "application/json",
                        data: JSON.stringify(docModifyApplyIds),
                        success: function(r){
                            if(r.code == 0){
                                layer.msg("操作成功", {icon: 1});
		...
		getInfo: function(docModifyApplyId){
			$.get(baseURL + "doc_manage/docmodifyapply/info/"+docModifyApplyId, function(r){
                vm.docModifyApply = r.docModifyApply;
            });
		},
       ...
       clickReject: function (docModifyApplyId){
            vm.showList = false;
            vm.showInfo = true;
            vm.approveMsg.isApproved = 0;
            vm.title = "驳回理由";
            vm.approveMsg.docModifyApplyId = docModifyApplyId;
            vm.approveMsg.approveTime= new Date().toISOString().slice(0, 19).replace('T', ' ');
        },


        clickApprove: function (docModifyApplyId){
            vm.showList = false;
            vm.showInfo = true;
        ...
        handleApproveOrReject: function (event){
            console.log("Handle ! TABLE ID: "+ vm.approveMsg.docModifyApplyId );
            $('#btnApproveOrRejectModify').button('loading').delay(1000).queue(function() {
                var url = "doc_manage/docmodifyapply/approve_or_reject_modify";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
                    contentType: "application/json",
                    ...
                    success: function(r){
                        if(r.code === 0){
                            ...
                            $('#btnApproveOrRejectModify').button('reset');
                            $('#btnApproveOrRejectModify').dequeue();
                        }else{
                            layer.alert(r.msg);
                            $('#btnApproveOrRejectModify').button('reset');
                            $('#btnApproveOrRejectModify').dequeue();
