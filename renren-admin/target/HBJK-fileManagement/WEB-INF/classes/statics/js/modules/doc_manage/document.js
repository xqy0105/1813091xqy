$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'doc_manage/document/list?valid=true',
        datatype: "json",
        colModel: [
            {label: '', name: '', index: 'action', width: 70, sortable:false,formatter: cmgStateFormat},
            ...
            // { label: '浏览权限类型', name: 'viewPrif', index: 'view_pri', width: 80 },
...
function cmgStateFormat(cellvalue, options, rowObject) {
    const html =
        ...
        "<button type=\"button\" class='btn btn-warning' style='line-height: 0.6' onclick=\"borrowSingle(" + rowObject.docId +
        ",'"+rowObject.docName+"')\">借阅</button> &nbsp;"+
...
var vm = new Vue({
    ...
    methods: {
        ...
        // 查询栏中的 借阅按钮
        clickBorrow: function (docId,docName) {
            vm.showList=false;
            vm.showBorrow=true;
            vm.showDetail=false;
            vm.title = "借阅";
