$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
            // {label:'操作',name:'act',index:'act', width:75,sortable:false},
            { label: '角色ID', name: 'roleId', index: "role_id", width: 30, key: true },
            {label: '', name: '', index: 'action', width: 60, sortable:false,formatter: cmgStateFormat},
...
function cmgStateFormat(cellvalue, options, rowObject) {
    ...
    //     "</div>"
    var html =
        "<button type=\"button\" class='btn btn-info' style='line-height: 0.6' onclick=\"updateSingle(" + options.rowId +")\">修改</button> &nbsp;" +
        "<button type=\"button\" class='btn btn-danger' style='line-height: 0.6' onclick=\"delSingle(" + options.rowId +")\">删除</button>" ;
